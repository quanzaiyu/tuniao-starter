...
export default {
  ...
  props: {
    ...
    selectRoot: { type: Boolean, default: false }, // 是否可以选择根级
    showIndexes: { type: Array, default: () => ([]) }, // 要显示的ID
  ...
  computed: {
    canSelect() {
      return item => {
        if (this.showIndexes.includes(Number(item.source.value))) {
          return true
        }
        return this.selectParent ? this.selectRoot ? true : item.rank > 0 : item.lastRank
      }
    },
  },
  watch: {
